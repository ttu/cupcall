...
// Vertical slice architecture (CLAUDE.md): a feature's internals (domain, application,
// infrastructure, ui) are private. Other code may depend on it only through its index.ts
// barrel. dependency-cruiser has no cross-field backreference between `from`/`to` regexes,
// so one rule is generated per feature instead of a single generic pattern. Unlike the
// ESLint no-restricted-imports rule, this checks resolved paths, so it also catches
// relative-path bypasses (e.g. `../../other-feature/domain/x`).
// 'use client' files must not import a feature barrel whose index.ts also re-exports
// server-only modules (e.g. auth's barrel re-exports `auth`, which pulls in
...
module.exports = {
  forbidden: [
    ...
    },
    {
      ...
      from: {
        ...
        pathNot: [
          ...
          // Next.js App Router file-convention modules, discovered by filename, not import.
          '(^|/)apps/web/src/app/.*/?(?:sitemap|robots|manifest|icon|apple-icon|opengraph-image)[.]tsx?$',
        ],
      },
      to: {},
    },
    ...
    },
    {
      ...
      from: {
        path: '^()',
        // Config/tooling entry points legitimately import devDependencies (test
        // runners, Storybook, drizzle-kit, tsx scripts) that never ship to production.
        pathNot: [
      ...
      to: {
        ...
        // production code or are ignored by the runtime. 'npm' is excluded too:
        // with combinedDependencies on, a package declared as a real dependency in
        // its own workspace but also (redundantly) as a root-level devDependency
        // picks up both types - that's a duplicate-dep-types issue, not a
        // ships-a-devDependency-to-prod issue.
        dependencyTypesNot: ['type-only', 'npm'],
  ...
  options: {
    ...
    exclude: {
      ...
      path: ['(^|/)dist/', '(^|/)[.]next/', '(^|/)storybook-static/'],
    },
    ...
    // This is a pnpm workspace: shared devDependencies (vitest, typescript, ...) live only
    // in the root package.json, not duplicated into every workspace package.json.
    ...
    // A standalone tsconfig (not apps/web/tsconfig.json) because dependency-cruiser's
    // TypeScript config loader doesn't resolve its "extends" chain correctly across
    // the monorepo boundary. Only the "@/*" path alias is needed for resolution.
    tsConfig: {
...
// generated: dependency-cruiser@18.1.0 on 2026-07-17T21:36:50.354Z
