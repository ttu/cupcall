...
function SheetHeader({
  ...
  return (
    <div className="flex items-start justify-between gap-2 p-[16px_18px_10px]">
      <div className="flex flex-col gap-2 min-w-0">
        ...
        {!hasScore && match.kickoff && (
          <span className="text-xs font-semibold text-ink-muted">{formatDate(match.kickoff)}</span>
        )}
      </div>
      <button
        type="button"
