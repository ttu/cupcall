...
type UserPools = Awaited<ReturnType<typeof getUserPools>>;
...
export default async function JoinPage({ params, searchParams }: Props): Promise<ReactElement> {
  ...
  if (!pool) {
    if (!actor) return <InvalidInviteForGuest />;

    const [pools, myLoginToken] = await Promise.all([
  ...
  // ── Signed-in path ───────────────────────────────────────────────────────
  if (actor) {
    return renderSignedInInvite(pool, actor, token, error, isLateJoin);
  }
...
// ── Invalid invite (no matching pool for token) ────────────────────────────

/** Fetches the actor's pools for the "invalid invite" card; the list is non-critical, so a failure is swallowed. */
async function getJoinedPoolsSafely(actor: Actor): Promise<UserPools> {
  try {
    return await getUserPools(db, actor.userId);
  } catch {
...
/** Guest accounts (no email) need a personal login link; creates one on first use, otherwise reuses it. */
async function getOrCreateGuestLoginToken(actor: Actor): Promise<string | null> {
...
function InvalidInviteForSignedInUser({
  pools,
  actor,
  myLoginToken,
  baseUrl,
...
// ── Signed-in path (pool found) ─────────────────────────────────────────────

async function renderSignedInInvite(
  pool: NonNullable<Awaited<ReturnType<typeof getPoolByInviteTokenHash>>>,
  actor: Actor,
  token: string,
  ...
  return (
    <div className="flex flex-col gap-5">
      ...
      <p className="text-center text-xs text-ink-muted">
        ...
        <Link
          href={`/?callbackUrl=${encodeURIComponent(joinPath)}`}
