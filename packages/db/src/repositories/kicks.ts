 ...
 */
export async function recordKick(db: Database, poolId: PoolId, userId: UserId): Promise<void> {
  await db
...
export async function isKicked(db: Database, poolId: PoolId, userId: UserId): Promise<boolean> {
  const [row] = await db
...
export async function clearKick(db: Database, poolId: PoolId, userId: UserId): Promise<void> {
  await db
